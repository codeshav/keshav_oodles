...
class DatasetHandler():
    ...
    def add_annotations(self, dataset):
        ...
        if self.annotation_method == AnnotationMethod.MASTER_FILE:
            annotated_data = self.annotation_helper.annotations_from_master_file(dataset)
        else:
    ...
    def create_retraining_dataset(self, dataset_location, new_data, old_dataset):
        ...
        write_json(dataset_location + "/training_dataset.json", self.merge_training_datasets(old_dataset, dataset_location + "/annotated_dataset.json", ratio=5))
